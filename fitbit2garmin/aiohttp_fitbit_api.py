...
async def get_activity_heart_rate(
    session: aiohttp.ClientSession,
    bearer_token: str,
    url: str,
) -> bytes:
    headers = fitbit_api.get_custom_authorization_headers()
    async with _API_RATE_LIMITER, session.get(url, headers=headers) as res:
        return await res.read()


async def get_activity_calories(
    session: aiohttp.ClientSession,
    bearer_token: str,
    url: str,
) -> bytes:
    ...
    async with _API_RATE_LIMITER:
        async with session.get(url, headers=headers) as res:
            if res.status == 200:
                ...
                fallback_url = url.split("1min/time", 1)[0] + "1min.json"
                print(f"Falling back to truncated url for calories: {fallback_url}")
